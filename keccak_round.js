...
function transpose(array) {
  ...
  for (let i = 0; i < 5; i++) {
    for (let j = 0; j < 5; j++) {
      result[(i * 5 + j) * 2] = array[(j * 5 + i) * 2];
      result[(i * 5 + j) * 2 + 1] = array[(j * 5 + i) * 2 + 1];
    }
  }
  return result;
}
...
function format64bitArray(arr) {
  const result = [];
  arr.forEach((v, i) => {
    const idx = Math.floor(i / 2);
    result[idx] ||= "";
    result[idx] = formatNumber(v).padStart(8, "0") + result[idx];
  });
  return result;
}
...
(function () {
  ...
  class Keccak {
    constructor(bits, padding, outputBits) {
      console.log("keccak initialized", { bits, padding, outputBits });
      this.blocks = [];
      this.s = [];
    ...
    update(message) {
      ...
      while (index < length) {
        ...
        if (i >= byteCount) {
          console.log("got Pi", blocks.slice(0, blockCount));
    ...
    finalize() {
      ...
      console.log("got last Pi", format64bitArray(blocks.slice(0, blockCount)));
      for (i = 0; i < blockCount; ++i) {
        s[i] ^= blocks[i];
      }
    ...
    hex() {
      ...
      var blockCount = this.blockCount,
        ...
        i = 0,
        j = 0;
      console.log("hex, s", format64bitArray(s));
      var hex = "",
        block;
